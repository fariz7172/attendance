...
@section('content')
    <div class="page-header">
        ...
        <p class="page-description">Buat data jabatan baru</p>
    </div>
            ...
            <form action="{{ route('admin.positions.store') }}" method="POST" style="max-width: 600px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                           value="{{ old('name') }}" placeholder="Contoh: Manager, Staff, Supervisor">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="meal_allowance" class="form-control currency-input @error('meal_allowance') is-invalid @enderror" 
                           value="{{ old('meal_allowance', 0) }}" required>
                    @error('meal_allowance')
                        <div style="color: #ef4444; font-size: 12px; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                    <small style="color: var(--color-text-muted);">Dihitung per kehadiran (On Time/Late/Early)</small>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="absent_fee" class="form-control currency-input @error('absent_fee') is-invalid @enderror" 
                           value="{{ old('absent_fee', 0) }}" required>
                    @error('absent_fee')
                        <div style="color: #ef4444; font-size: 12px; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                </div>
